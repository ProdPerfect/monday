...
from monday.query_joins import mutate_item_query, get_item_query, update_item_query, get_item_by_id_query, \
    ...
    archive_item_query, move_item_to_group_query, get_subitems_by_item_query, get_subcolumns_by_item_query
...
class ItemResource(BaseResource):
    ...
    def fetch_subitems_by_item(self, board_id, item_id):
        query = get_subitems_by_item_query(board_id=board_id, item_id=item_id)
        return self.client.execute(query)

    def fetch_subcolumns_by_item(self, board_id, item_id):
        query = get_subcolumns_by_item_query(board_id=board_id, item_id=item_id)
        return self.client.execute(query)
